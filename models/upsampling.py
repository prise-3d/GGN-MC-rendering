...
class Encoder(torch.nn.Module):
  def __init__(self, input_chanels, feature_maps):
    ...
    self.encoder = torch.nn.Sequential(
                                       ...
                                       torch.nn.Dropout(0.3),
                                       torch.nn.Conv2d(feature_maps * 16, feature_maps * 32, kernel_size=3, stride=1),
                                       torch.nn.LeakyReLU(0.2, inplace=False),
                                       torch.nn.BatchNorm2d(feature_maps * 32),
                                       #Flatten()
                                      )
...
class Decoder(torch.nn.Module):
  def __init__(self, feature_maps):
    ...
    self.decoder = torch.nn.Sequential(
                                    #UnFlatten(),
                                    torch.nn.ConvTranspose2d(feature_maps * 32, feature_maps * 16, kernel_size=3, stride=1),
                                    torch.nn.ReLU(),
                                    torch.nn.BatchNorm2d(feature_maps * 16),
                                    torch.nn.Dropout(0.3),
